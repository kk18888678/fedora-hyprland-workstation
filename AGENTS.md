...
# 39. Workstation Configuration and State Architecture
...
- **Plan Finalization and Deterministic Integrity**: After planning and before Review/Reconciliation, the plan is finalized with a deterministic SHA-256 integrity fingerprint (`finalize_plan`). The Reconciler recomputes and verifies the fingerprint before executing actions (`validate_plan`), failing closed on unfinalized, malformed, or modified plans. This protects the internal Review -> Reconciler consistency boundary against accidental or unexpected plan mutation. It is an internal state consistency check, not a cryptographic authentication boundary against code already executing inside the installer process.
...
- **Presence != Default**: Application presence and default role preferences are independent. Multiple providers of a role (e.g. browsers) may coexist; default association is explicit and configurable. Desired role preferences generate executable `CHANGE_DEFAULT` actions only when a real system-default adapter is registered (`role_has_default_adapter`); preference modeling and mutation execution remain strictly decoupled.
