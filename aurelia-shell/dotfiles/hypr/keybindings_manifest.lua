-- Single source of truth for all workstation keybindings and shortcuts.
-- Both Hyprland session registration (keybind.lua) and user-facing Hotkeys help
-- (workstation-hotkeys) are dynamically derived from this manifest.

local M = {}

M.mainMod = "SUPER"

M.categories = {
    "Applications & Launchers",
    "Window Management",
    "Workspaces",
    "Window Focus & Resizing",
    "Audio & Media Controls",
    "Mouse Controls",
}

M.bindings = {
    -- Applications & Launchers
    {
        id = "terminal",
        priority = 20,
        editable = true,
        category = "Applications & Launchers",
        key = "SUPER + RETURN",
        action_type = "exec",
        command = "gtk-launch -- kitty.desktop",
        command_argv = { "gtk-launch", "--", "kitty.desktop" },
        desktop_id = "kitty.desktop",
        display_key = "Super + Return",
        description = "Terminal",
        runnable = true,
    },
    {
        id = "default_agent",
        priority = 25,
        editable = true,
        category = "Applications & Launchers",
        key = "SUPER + SHIFT + CTRL + A",
        action_type = "exec",
        command = "workstation-ai launch",
        command_argv = { "workstation-ai", "launch" },
        display_key = "Super + Shift + Ctrl + A",
        description = "Default AI agent",
        runnable = true,
    },
    {
        id = "file_manager",
        priority = 30,
        editable = true,
        category = "Applications & Launchers",
        key = "SUPER + E",
        action_type = "exec",
        command = "gtk-launch -- org.gnome.Nautilus.desktop",
        command_argv = { "gtk-launch", "--", "org.gnome.Nautilus.desktop" },
        desktop_id = "org.gnome.Nautilus.desktop",
        display_key = "Super + E",
        description = "Files",
        runnable = true,
    },
    {
        id = "browser",
        priority = 40,
        editable = true,
        category = "Applications & Launchers",
        key = "SUPER + B",
        action_type = "exec",
        command = "gtk-launch -- chromium-browser.desktop",
        command_argv = { "gtk-launch", "--", "chromium-browser.desktop" },
        desktop_id = "chromium-browser.desktop",
        display_key = "Super + B",
        description = "Browser",
        runnable = true,
    },
    {
        id = "keybindings",
        priority = 50,
        editable = true,
        category = "Applications & Launchers",
        key = "SUPER + K",
        action_type = "exec",
        command = "aurelia-shell-keybindings",
        command_argv = { "aurelia-shell-keybindings" },
        desktop_id = "aurelia-shell-keybindings.desktop",
        display_key = "Super + K",
        description = "Keybindings",
        runnable = true,
    },
    {
        id = "desktop_settings",
        priority = 60,
        editable = true,
        category = "Applications & Launchers",
        key = "SUPER + T",
        action_type = "exec",
        command = "aurelia settings toggle",
        command_argv = { "aurelia", "settings", "toggle" },
        desktop_id = "aurelia.settings",
        display_key = "Super + T",
        description = "Desktop Settings",
        runnable = true,
    },
    {
        id = "lock_screen",
        priority = 70,
        editable = true,
        category = "Applications & Launchers",
        key = "SUPER + L",
        action_type = "exec",
        command = "noctalia msg screen-lock",
        command_argv = { "noctalia", "msg", "screen-lock" },
        display_key = "Super + L",
        description = "Lock Screen",
        runnable = true,
    },

    -- Window Management
    {
        id = "window_close",
        priority = 100,
        editable = true,
        category = "Window Management",
        key = "SUPER + Q",
        action_type = "dispatch_close",
        display_key = "Super + Q",
        description = "Close Window",
        runnable = false,
    },
    {
        id = "window_toggle_fullscreen",
        priority = 110,
        editable = true,
        category = "Window Management",
        key = "SUPER + F",
        action_type = "dispatch_fullscreen",
        display_key = "Super + F",
        description = "Fullscreen",
        runnable = false,
    },
    {
        id = "window_toggle_float",
        priority = 120,
        editable = true,
        category = "Window Management",
        key = "SUPER + W",
        action_type = "dispatch_float",
        display_key = "Super + W",
        description = "Toggle Floating",
        runnable = false,
    },
    {
        id = "window_cycle_focus",
        priority = 130,
        editable = true,
        category = "Window Management",
        key = "ALT + tab",
        action_type = "dispatch_cycle",
        display_key = "Alt + Tab",
        description = "Cycle Window Focus",
        runnable = false,
    },

    -- Workspaces
    {
        id = "workspaces_switch_1_10",
        priority = 200,
        editable = false,
        category = "Workspaces",
        generator = "workspaces_1_10",
        display_key = "Super + 1 .. 9, 0",
        description = "Workspace 1–10",
        runnable = false,
    },
    {
        id = "workspaces_move_1_10",
        priority = 210,
        editable = false,
        category = "Workspaces",
        generator = "workspaces_move_1_10",
        display_key = "Super + Shift + 1 .. 9, 0",
        description = "Move Window to Workspace 1–10",
        runnable = false,
    },
    {
        id = "workspace_prev",
        priority = 220,
        editable = true,
        category = "Workspaces",
        key = "SUPER + CTRL + left",
        action_type = "focus_workspace_relative",
        workspace = "r-1",
        display_key = "Super + Ctrl + Left",
        description = "Previous Workspace",
        runnable = false,
    },
    {
        id = "workspace_next",
        priority = 230,
        editable = true,
        category = "Workspaces",
        key = "SUPER + CTRL + right",
        action_type = "focus_workspace_relative",
        workspace = "r+1",
        display_key = "Super + Ctrl + Right",
        description = "Next Workspace",
        runnable = false,
    },
    {
        id = "workspace_touchpad_swipe",
        priority = 240,
        editable = false,
        immutable = true,
        category = "Workspaces",
        action_type = "gesture",
        trigger_type = "gesture",
        keyboard_bindable = false,
        display_key = "3-Finger Horizontal Swipe",
        description = "Swipe Workspaces (Touchpad)",
        runnable = false,
    },

    -- Window Focus & Resizing
    {
        id = "focus_left",
        priority = 300,
        editable = true,
        category = "Window Focus & Resizing",
        key = "SUPER + left",
        action_type = "focus",
        direction = "left",
        display_key = "Super + Left",
        description = "Focus Window Left",
        runnable = false,
    },
    {
        id = "focus_right",
        priority = 301,
        editable = true,
        category = "Window Focus & Resizing",
        key = "SUPER + right",
        action_type = "focus",
        direction = "right",
        display_key = "Super + Right",
        description = "Focus Window Right",
        runnable = false,
    },
    {
        id = "focus_up",
        priority = 302,
        editable = true,
        category = "Window Focus & Resizing",
        key = "SUPER + up",
        action_type = "focus",
        direction = "up",
        display_key = "Super + Up",
        description = "Focus Window Up",
        runnable = false,
    },
    {
        id = "focus_down",
        priority = 303,
        editable = true,
        category = "Window Focus & Resizing",
        key = "SUPER + down",
        action_type = "focus",
        direction = "down",
        display_key = "Super + Down",
        description = "Focus Window Down",
        runnable = false,
    },
    {
        id = "resize_window_left",
        priority = 310,
        editable = true,
        category = "Window Focus & Resizing",
        key = "SUPER + SHIFT + left",
        action_type = "exec_resize",
        resize_args = "-30 0",
        display_key = "Super + Shift + Left",
        description = "Resize Window Left",
        runnable = false,
    },
    {
        id = "resize_window_right",
        priority = 311,
        editable = true,
        category = "Window Focus & Resizing",
        key = "SUPER + SHIFT + right",
        action_type = "exec_resize",
        resize_args = "30 0",
        display_key = "Super + Shift + Right",
        description = "Resize Window Right",
        runnable = false,
    },
    {
        id = "resize_window_up",
        priority = 312,
        editable = true,
        category = "Window Focus & Resizing",
        key = "SUPER + SHIFT + up",
        action_type = "exec_resize",
        resize_args = "0 -30",
        display_key = "Super + Shift + Up",
        description = "Resize Window Up",
        runnable = false,
    },
    {
        id = "resize_window_down",
        priority = 313,
        editable = true,
        category = "Window Focus & Resizing",
        key = "SUPER + SHIFT + down",
        action_type = "exec_resize",
        resize_args = "0 30",
        display_key = "Super + Shift + Down",
        description = "Resize Window Down",
        runnable = false,
    },

    -- Audio & Media Controls
    {
        id = "volume_raise",
        priority = 400,
        editable = true,
        category = "Audio & Media Controls",
        key = "XF86AudioRaiseVolume",
        action_type = "exec_locked",
        command = "wpctl set-volume @DEFAULT_AUDIO_SINK@ 5%+",
        command_argv = { "wpctl", "set-volume", "@DEFAULT_AUDIO_SINK@", "5%+" },
        display_key = "Volume Up Key",
        description = "Volume Up",
        runnable = true,
    },
    {
        id = "volume_lower",
        priority = 401,
        editable = true,
        category = "Audio & Media Controls",
        key = "XF86AudioLowerVolume",
        action_type = "exec_locked",
        command = "wpctl set-volume @DEFAULT_AUDIO_SINK@ 5%-",
        command_argv = { "wpctl", "set-volume", "@DEFAULT_AUDIO_SINK@", "5%-" },
        display_key = "Volume Down Key",
        description = "Volume Down",
        runnable = true,
    },
    {
        id = "volume_mute_toggle",
        priority = 402,
        editable = true,
        category = "Audio & Media Controls",
        key = "XF86AudioMute",
        action_type = "exec_locked",
        command = "wpctl set-mute @DEFAULT_AUDIO_SINK@ toggle",
        command_argv = { "wpctl", "set-mute", "@DEFAULT_AUDIO_SINK@", "toggle" },
        display_key = "Mute Key",
        description = "Toggle Mute",
        runnable = true,
    },
    {
        id = "media_play_pause",
        priority = 410,
        editable = true,
        category = "Audio & Media Controls",
        key = "XF86AudioPlay",
        action_type = "exec_locked",
        command = "playerctl play-pause",
        command_argv = { "playerctl", "play-pause" },
        display_key = "Play / Pause Key",
        description = "Play / Pause",
        runnable = true,
    },
    {
        id = "media_next_track",
        priority = 411,
        editable = true,
        category = "Audio & Media Controls",
        key = "XF86AudioNext",
        action_type = "exec_locked",
        command = "playerctl next",
        command_argv = { "playerctl", "next" },
        display_key = "Next Track Key",
        description = "Next Track",
        runnable = true,
    },
    {
        id = "media_prev_track",
        priority = 412,
        editable = true,
        category = "Audio & Media Controls",
        key = "XF86AudioPrev",
        action_type = "exec_locked",
        command = "playerctl previous",
        command_argv = { "playerctl", "previous" },
        display_key = "Previous Track Key",
        description = "Previous Track",
        runnable = true,
    },

    -- Mouse Controls
    {
        id = "mouse_window_drag",
        priority = 500,
        editable = false,
        category = "Mouse Controls",
        key = "SUPER + mouse:272",
        action_type = "mouse_drag",
        trigger_type = "mouse",
        keyboard_bindable = false,
        mouse = true,
        display_key = "Super + Left Mouse Drag",
        description = "Move Window (Mouse)",
        runnable = false,
    },
    {
        id = "mouse_window_resize",
        priority = 501,
        editable = false,
        category = "Mouse Controls",
        key = "SUPER + mouse:273",
        action_type = "mouse_resize",
        trigger_type = "mouse",
        keyboard_bindable = false,
        mouse = true,
        display_key = "Super + Right Mouse Drag",
        description = "Resize Window (Mouse)",
        runnable = false,
    },
}

-- First-party plugin bindings are declared by each plugin and merged into the
-- same effective registry consumed by both the UI and Hyprland provider.
local function load_aurelia_plugin_bindings()
    local source = debug.getinfo(1, "S").source or ""
    source = source:gsub("^@", "")
    local shell_root = source:gsub("/dotfiles/hypr/keybindings_manifest%.lua$", "")
    local plugin_dir = shell_root .. "/plugins"
    local function shell_quote(path)
        return "'" .. path:gsub("'", "'\\''") .. "'"
    end

    local known_ids = {}
    for _, item in ipairs(M.bindings) do
        if type(item) == "table" and type(item.id) == "string" then
            known_ids[item.id] = true
        end
    end

    local handle = io.popen(
        "find " .. shell_quote(plugin_dir) ..
        " -mindepth 2 -maxdepth 2 -type f -name 'keybindings.lua' " ..
        "-printf '%p\\n'  | sort"
    )
    if not handle then return end

    for declaration_path in handle:lines() do
        local ok, declarations = pcall(dofile, declaration_path)
        if ok and type(declarations) == "table" then
            for _, item in ipairs(declarations) do
                if type(item) == "table" and type(item.id) == "string" and not known_ids[item.id] then
                    table.insert(M.bindings, item)
                    known_ids[item.id] = true
                end
            end
        end
    end
    handle:close()
end

load_aurelia_plugin_bindings()

return M
