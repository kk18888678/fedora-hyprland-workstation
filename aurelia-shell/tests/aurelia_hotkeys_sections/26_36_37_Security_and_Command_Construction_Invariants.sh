...
ipc_methods="$(grep -E 'function [a-zA-Z0-9_]+\(\)' "$ROOT/shell.qml" | sed 's/^[[:space:]]*function //;s/(.*//' | tr '\n' ' ')"
...
if ! grep -E '(bash -c|sh -c|eval )' "$ROOT/components/hotkeys/HotkeysModel.qml" >/dev/null; then
