...
if [[ "$matrix_manifest_count" -eq 29 ]]; then
    pass "[static] first-party manifest enumeration finds all 29 Aurelia plugins"
else
    fail "[static] first-party manifest enumeration expected 29 plugins, found $matrix_manifest_count"
fi
