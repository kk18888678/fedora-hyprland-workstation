...
if [[ "$matrix_manifest_count" -eq 24 ]]; then
    pass "[static] first-party manifest enumeration finds all 24 Aurelia plugins"
else
    fail "[static] first-party manifest enumeration expected 24 plugins, found $matrix_manifest_count"
fi
