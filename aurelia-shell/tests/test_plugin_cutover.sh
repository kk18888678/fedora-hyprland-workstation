...
if grep -q 'function normalize(candidate)' "$config_root" &&
   ...
   grep -q 'ShellConfigFileBoundary' "$config_root" &&
   grep -q 'atomicWrites: true' "$ROOT/services/ShellConfigFileBoundary.qml" &&
...
if [[ ! -x /usr/bin/qs || ! -x /usr/bin/timeout ]]; then
    skip "[isolated-runtime] compatibility cutover QuickShell fixtures (qs or timeout unavailable)"
    return 0
fi

cutover_root="$(mktemp -d)"
trap 'rm -rf -- "$cutover_root"  || true' RETURN
cutover_result="$cutover_root/probe-result.json"
cutover_log="$cutover_root/probe.log"
cutover_status=0
: >"$cutover_result"
printf '%s\n' '{}' >"$cutover_root/empty-shell.json"
...
if [[ "$cutover_status" -eq 0 ]] && [[ -s "$cutover_result" ]] &&
   runtime_log_is_environment_only "$cutover_log" &&
   jq -e '
   ...
   ' "$cutover_result" >/dev/null; then
    ...
    details="$(tail -n 32 "$cutover_log"  || true)"
...
if [[ "$legacy_status" -eq 0 ]] && [[ -s "$legacy_result" ]] &&
   runtime_log_is_environment_only "$legacy_log" &&
...
if [[ "$second_status" -eq 0 ]] && [[ -s "$second_result" ]] &&
   ...
   jq -e '.migrationNeededBefore == false and .migrationReturn == "ok" and .migrationNeededAfter == false' \
    ...
    details="$(tail -n 32 "$second_log"  || true)"
