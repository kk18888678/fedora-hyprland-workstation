...
eval_check="$(grep -v '^[[:space:]]*#' "$ROOT/bin/workstation-keybindings" | grep -E '\beval\b|sh -c'  || true)"
...
DUMMY_EOF
    ...
    PATH="$sb:$PATH" "$ROOT/bin/workstation-keybindings" run terminal >/dev/null
...
(
    ...
    "$ROOT/bin/workstation-keybindings" run terminal >/dev/null || true
