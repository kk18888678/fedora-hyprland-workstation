...
# 33.4: Aurelia preferences owned under core/preferences.lua (and dotfiles/hypr/aurelia_preferences.lua is removed)
if [[ -f "$ROOT/core/preferences.lua" && ! -f "$ROOT/dotfiles/hypr/aurelia_preferences.lua" ]]; then
    pass "33.4 Aurelia preferences canonically owned under core/preferences.lua with hypr/ removal"
else
...
if ! grep -q 'config/hypr' "$ROOT/bin/workstation-aurelia" && \
   ! grep -q 'config/hypr' "$ROOT/core/preferences.lua"; then
