...
if grep -q 'f:seek("end")' "$ROOT/core/preferences.lua" && \
   ! grep -q 'read_all_lines' "$ROOT/core/preferences.lua"; then
    pass "35.5 lightweight bounded logging writes directly and checks size with O(1) seek"
else
    fail "35.5 O(1) seek logging missing in core/preferences.lua"
fi
