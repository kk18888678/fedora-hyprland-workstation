...
if [[ -L "$ROOT/dotfiles/aurelia" ]] &&
   [[ "$(readlink "$ROOT/dotfiles/aurelia")" == "../aurelia-shell" ]]; then
    pass "legacy dotfiles/aurelia path is a non-duplicating compatibility symlink"
else
    fail "legacy dotfiles/aurelia compatibility symlink is missing or points elsewhere"
fi
...
if grep -q '/usr/local/bin/aurelia-shell-keybindings' "$plugin_root/ui/KeybindingsModel.qml" &&
   ...
   grep -q '/usr/local/bin/aurelia-launch-shell' "$ROOT/dotfiles/hypr/startup.lua"; then
